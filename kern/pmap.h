...
// takes a kernel virtual address (an address that points above KERNBASE, 
// where the machines's maximum 256MB of physical memory is mapped), 
// and returns the corresponding physical address. It panics if passed with a non-kernel virtual address.
...
// It panics if passed with an invalid physical address.
...
struct PageInfo *page_lookup(pde_t *pgdir, void *va, pde_t **pte_store);
