...
static int kbd_proc_data(void)
{
	...
	// process special keys, reboot: ctl-alt-del
...
// general device-independent console code, manage the console input buffer
...
// return the next input character from the console, or 0 if nothing waiting
int cons_getc(void)
{
	int c;

	// poll for any pending input characters so that this function works even when interrupts are disabled
	serial_intr();
	kbd_intr();
