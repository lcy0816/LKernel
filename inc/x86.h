...
static inline uint8_t inb(int port) // input byte from immediate port into AL
{
	uint8_t data;
...
static inline uint16_t inw(int port) // input byte from immediate port into AX
{
	uint16_t data;
...
static inline uint32_t rcr2(void) // read cr2
{
	uint32_t val;
...
static inline void lcr3(uint32_t val) // set cr3
{
...
static inline uint32_t rcr4(void) // read cr4
{
	uint32_t cr4;
...
static inline uint32_t xchg(volatile uint32_t *addr, uint32_t newal)
{
	...
	// "+m" denotes a read-modify-write operand
